...
def download(url, file_name):
    ...
    save_path = "/users/saedunu/ganesh/" + file_name
...
def bed_to_txt(bed_file):
    reference_fasta = "/users/saedunu/ganesh/reference/hg38.fa"
    bed_filename = "/users/saedunu/ganesh/" + bed_file
    output_txt = input("Enter the file name to extract txt file from bed file-{}: ".format(bed_file))
    output_filename = "/users/saedunu/ganesh/" + output_txt
...
for i in range(len(bedtool_txt_unaccess_list)):
	...
	final_optimal_length, cropped_unaccessible_file = crop_unaccess_sequences(bedtool_txt_unaccess_list[i], unaccess_output_filename, optimal_length_list[i])
